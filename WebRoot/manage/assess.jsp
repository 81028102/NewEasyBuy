...
$(function(){
	...
	$("#delete").click(function(){
		...
		ea_Upid=id.substring(1);
		if(ea_Upid!=""){
			if(confirm("确定要删除吗?请谨慎删除哦!")){
				location.href="<%=request.getContextPath() %>/productServlet?action=del&ea_upid="+ea_Upid;
			}
		}else{
				alert("对不起,请指定删除!");
		}
	...
	if(n==2){document.title="轻松购 - 评价管理";}
...
<body>
	...
	<div id="main" class="wrap">
		...
		<div class="main">
			<h2>评价管理<a style="color: red;text-decoration: none;margin-left: 410px;">温馨提示:请谨慎删除,删除后不可恢复哦!</a></h2>
			<div class="manage">
			<form action="<%=request.getContextPath()%>/productServlet?action=looks" method="post">
			<table style="margin-left: -11px;"><tr align="right"><td>
  			<input type="hidden" name="cpage" id="cpage" value="${cpage}" />
  			评价内容:<input type="text" class="text" name="ea_assess" id="ea_assess" value="${ea_assess}" placeholder="请输入评价内容(支持模糊查询)" autocomplete="off"/>
  			<label class="ui-blue"><input type="submit" class="point" value="搜索"/></label>
  			</td></tr></table></form>
  			<c:if test="${not empty looksList }">
			<table><tr align="left"><td><label class="ui-blue"><input type="button" value="反选" id="fx" class="point"/></lable></td>  <td><label class="ui-blue"><input type="button"
				...
				id="delete" class="point"/></label></td> <td><label class="ui-blue"><a class="manageDel" href="<%=request.getContextPath() %>/productServlet?action=delAssessAll" style="text-decoration:none;"><input type="button" style="color:orange;" value="删除全部" class="point"/></a></label></td></tr></table>
				</c:if>
				<table class="list">
					<tr>
						...
						<th>评价ID</th>
						<th>评价人</th>
						<th><img src="<%=request.getContextPath() %>/images/write.gif"/>评价内容</th>
						<th><img src="<%=request.getContextPath() %>/images/date.gif"/>评价时间</th>
						<th>操作</th>
					</tr>
					<c:if test="${not empty looksList }">
					<c:forEach var="looksList" items="${looksList }">
						<tr><!-- class="first w4 c" -->
							<td><input type="checkbox" class="cks" value="${looksList.ea_Upid }"></td>
							<td style="text-align:center;">${looksList.ea_Upid }</td>
							<td class="w1 c">${looksList.ea_Nike_name }</td>
							<td style="text-align:center;">${looksList.ea_Assess }</td>
							<td class="w2 c">${looksList.ea_Create_time }</td>
							<td class="w1 c">
								<label class="ui-blue"><a class="manageDel" href="<%=request.getContextPath() %>/productServlet?action=del&ea_upid=${looksList.ea_Upid }"><input type="button" value="删除" class="point"/></a></label>
							</td>
						</tr>
					</c:forEach>
  		 	 <tr>
    		<td colspan="10" align="right">
    		当前是<a style="color: blue;text-decoration:none" >${cpage}</a>/<a style="color: blue;text-decoration:none" >${totalPage}</a>页   共<a style="color: blue;text-decoration:none" >${countass}</a>条
