...
<body>
<%session.removeAttribute("cmessage"); %>
<%session.removeAttribute("csb"); %>
<%session.removeAttribute("cbool"); %>
...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>栏目管理<a style="color: red;text-decoration: none;margin-left: 410px;">温馨提示:请谨慎删除,删除后不可恢复哦!</a></h2>
		<div class="manage">
		<form action="<%=request.getContextPath()%>/NewsServlet?action=columns" method="post">
			<table style="margin-left: -11px;"><tr align="right"><td>
  			<input type="hidden" name="cpage" id="cpage" value="${cpage}" />
  			栏目名称:<input type="text" class="text" name="enc_Name" id="enc_Name" value="${enc_Name}" placeholder="请输入栏目名称(支持模糊查询)" autocomplete="off"/>
  			...
  			<c:if test="${not empty listColumn }">
				...
				id="delete" class="point"/></label></td> <td><label class="ui-blue"><a class="manageDel" href="<%=request.getContextPath() %>/NewsServlet?action=delNewsColumnAll" style="text-decoration:none;"><input type="button" style="color:orange;" value="删除全部" class="point"/></a></label></td></tr></table>
				</c:if>
			<table class="list">
			<tr>	
			...
			<c:if test="${not empty listColumn }">
			<c:forEach var="i" items="${listColumn }" varStatus="status">
				<tr>
					<td><input type="checkbox" class="cks" value="${i.enc_Id }"></td>
					<td style="text-align:center;">${i.enc_Id }</td>
					<td style="text-align:center;">${i.enc_Name }</td>
					<td style="text-align:center;">${i.enc_Create_time }</td>
					<td style="text-align:center;">
					<label class="ui-blue"><a class="manageUpd" href="<%=request.getContextPath()%>/NewsServlet?action=getNewsColumnById&enc_id=${i.enc_Id }"><input type="button" class="point" value="修改"/></a></label>
					<label class="ui-blue"><a class="manageDel" href="<%=request.getContextPath()%>/NewsServlet?action=delNewsColumnById&enc_id=${i.enc_Id }"><input type="button" class="point" value="删除"/></a></label></td>
				</tr>
				</c:forEach>
    	 <tr>
    		<td colspan="10" align="right">
    		当前是<a style="color: blue;text-decoration:none" >${cpage}</a>/<a style="color: blue;text-decoration:none" >${totalPage}</a>页   共<a style="color: blue;text-decoration:none" >${countColumn}</a>条
