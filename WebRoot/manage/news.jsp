...
$(function(){
	...
	$("#delete").click(function(){
		...
		en_Id=id.substring(1);
		if(en_Id!=""){
			if(confirm("确定要删除吗?请谨慎删除哦!")){
				alert("操作成功!");
				location.href="<%=request.getContextPath() %>/NewsServlet?action=del&en_id="+en_Id;
			}
		}else{
				alert("对不起,请指定删除!");
		}
...
<div id="main" class="wrap">
	...
	<div class="main">
		...
		<div class="manage">
		<form action="<%=request.getContextPath()%>/NewsServlet?action=first" method="post">
			<table style="margin-left: -11px;"><tr align="right"><td>
  			...
  			<c:if test="${not empty lists }">
				...
				id="delete" class="point"/></label></td> <td><label class="ui-blue"><a class="manageDel" href="<%=request.getContextPath() %>/NewsServlet?action=delNewsAll" style="text-decoration:none;"><input type="button" style="color:orange;" value="删除全部" class="point"/></a></label></td></tr></table>
				</c:if>
			<table class="list">
			<tr>	
			...
			<c:forEach var="i" items="${lists }" varStatus="status">
				<tr>
					<td><input type="checkbox" class="cks" value="${i.en_Id }"></td>
					<td style="text-align:center;">${i.en_Id }</td>
					<td style="text-align:center;">${i.en_Title }</td>
					...
					<label class="ui-blue"><a class="manageUpd" href="<%=request.getContextPath()%>/NewsServlet?action=two&en_id=${i.en_Id }"><input type="button" class="point" value="修改"/></a></label>
					<label class="ui-blue"><a class="manageDel" href="<%=request.getContextPath()%>/NewsServlet?action=del&en_id=${i.en_Id }"><input type="button" class="point" value="删除"/></a></label></td>
				</tr>
				</c:forEach>
    	 <tr>
    		<td colspan="10" align="right">
    		当前是<a style="color: blue;text-decoration:none" >${cpage}</a>/<a style="color: blue;text-decoration:none" >${totalPage}</a>页   共<a style="color: blue;text-decoration:none" >${counttt}</a>条
